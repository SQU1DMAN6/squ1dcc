...
func TestDefine(t *testing.T) {
	...
	a := global.Define("a")
	if a != expected["a"] {
		t.Errorf("Expected a as %+v, got %+v", expected["a"], a)
	}

	b := global.Define("b")
	if b != expected["b"] {
		t.Errorf("Expected b as %+v, got %+v", expected["b"], b)
	}
	...
	if c != expected["c"] {
		t.Errorf("Expected c as %+v, got %+v", expected["c"], c)
	}

	d := firstLocal.Define("d")
	if d != expected["d"] {
		t.Errorf("Expected d as %+v, got %+v", expected["d"], d)
	}
	...
	if e != expected["e"] {
		t.Errorf("Expected e as %+v, got %+v", expected["e"], e)
	}

	f := secondLocal.Define("f")
	if f != expected["f"] {
		t.Errorf("Expected f as %+v, got %+v", expected["f"], f)
	}
}
...
func TestDefineResolveBuiltins(t *testing.T) {
	...
	for _, table := range []*SymbolTable{global, firstLocal, secondLocal} {
		for _, sym := range expected {
			...
			if result != sym {
				t.Errorf("Expected %s to resolve to %+v, gut %+v",
					sym.Name, sym, result)
			}
		}
	}
...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(tt.table.FreeSymbols) != len(tt.expectedFreeSymbols) {
			t.Errorf("Wrong number of free symbols. Expected %d, got %d",
				len(tt.expectedFreeSymbols), len(tt.table.FreeSymbols))
			continue
		}
		...
		for i, sym := range tt.expectedFreeSymbols {
			...
			if result != sym {
				t.Errorf("Wrong free symbol. Expected %+v, got %+v",
					result, sym)
			}
		}
	}
